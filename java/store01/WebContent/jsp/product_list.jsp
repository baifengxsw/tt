	...
	<body>
		...
		<div class="row" style="width:1210px;margin:0 auto;">
			...
			<div class="col-md-2" style="height:240px">
				...
				<p><a href="${pageContext.request.contextPath}/ProductServlet?method=findProductByPid&pid=${p.pid}" style='color:green'>冬瓜</a></p>
					<c:if test="${fn:length(p.pname)lt 27}">${p.pname}</c:if>
					<c:if test="${fn:length(p.pname)ge 27}">${fn:substring(p.pname,0,27)}</c:if>
