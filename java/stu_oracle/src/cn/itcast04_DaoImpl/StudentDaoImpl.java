...
public class StudentDaoImpl implements StudentDao {
	...
	public int insert(Student student) throws SQLException {
		QueryRunner runner = new QueryRunner(C3P0Utils.getDataSource());
		int ret = runner.update("insert into stu values(null,?,?,?,to_date(?,'yyyy-mm-dd'),?,?)",student.getSname(),
				student.getGender(),student.getPhone(),C3P0Utils.getDateString(student.getBirthday()),student.getHobby(),student.getInfo());
		return ret;
	}
	...
	public int update(Student stu) throws SQLException {
		QueryRunner runner = new QueryRunner(C3P0Utils.getDataSource());
		int ret = runner.update("update stu set sname=?, gender=?,phone=?,birthday =to_date(?,'yyyy-mm-dd'),hobby=?,info = ? where sid = ?",stu.getSname(),stu.getGender(),stu.getPhone()
				,C3P0Utils.getDateString(stu.getBirthday()),stu.getHobby(),stu.getInfo(),stu.getSid());
			return ret;
	}
