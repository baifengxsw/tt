package cn.itcast.UserPortraitMod;
...
import cn.itcast.UserPortraitMod.util.Config;
import cn.itcast.UserPortraitMod.util.TextArrayWritable;
...
public class UserDrawMapReduce {
    ...
    public static class MyReduce extends
        ...
        public void reduce(Text key, Iterable<TextArrayWritable> values,
            ...
            for (TextArrayWritable t : values) {
                ...
                if (flg) {
                    System.arraycopy(vals,0,res,0,res.length);
                    flg =false;
                }
                if (vals[3] != null) {
                    count = count + 1;
	...
	public static void main(String[] args) throws Exception {
		...
		FileInputFormat.addInputPath(job1, new Path("E:\\mr\\in"));// 输入路径
		FileOutputFormat.setOutputPath(job1, new Path("e:\\mr\\out"));// 输出路径
		...
		Boolean state1 = job1.waitForCompletion(true);
			if (state1) {
				...
				FileInputFormat.addInputPath(job2, new Path("e:\\mr\\out"));// 输入路径
				FileOutputFormat.setOutputPath(job2, new Path("e:\\mr\\result"));// 输出路径
